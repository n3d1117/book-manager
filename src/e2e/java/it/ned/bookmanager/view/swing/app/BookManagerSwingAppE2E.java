...
public class BookManagerSwingAppE2E extends AssertJSwingJUnitTestCase {
	...
	protected void onSetUp() {
		...
		application("it.ned.bookmanager.view.swing.app.BookManagerSwingApp")
				.withArgs("--mongo-replica-set-url=" + container.getReplicaSetUrl(), "--db-name=" + DB_NAME,
						"--db-author-collection=" + DB_AUTHOR_COLLECTION, "--db-book-collection=" + DB_BOOK_COLLECTION)
				.start();
	...
	public void testAddAuthorSuccess() {
		...
		await().atMost(TIMEOUT_SECONDS, TimeUnit.SECONDS).untilAsserted(() -> {
			assertThat(window.list("authorsList").contents()).anySatisfy(e -> assertThat(e).contains("James Joyce"));
			assertThat(window.comboBox("authorsCombobox").contents())
	...
	public void testAddAuthorError() {
		...
		await().atMost(TIMEOUT_SECONDS, TimeUnit.SECONDS).untilAsserted(
				() -> assertThat(window.label("authorErrorLabel").text()).contains(AUTHOR_FIXTURE_1.getId()));
	}

	@Test
	@GUITest
	public void testDeleteAuthorSuccess() {
		window.list("authorsList").selectItem(Pattern.compile(".*" + AUTHOR_FIXTURE_1.getName() + ".*"));
		window.button(JButtonMatcher.withName("deleteAuthorButton")).click();
		await().atMost(TIMEOUT_SECONDS, TimeUnit.SECONDS)
				.untilAsserted(() -> assertThat(window.list("authorsList").contents())
	...
	public void testAddBookSuccess() {
		...
		window.comboBox("authorsCombobox").selectItem(Pattern.compile(".*" + AUTHOR_FIXTURE_2.getName() + ".*"));
	...
	public void testAddBookError() {
		...
		window.comboBox("authorsCombobox").selectItem(Pattern.compile(".*" + AUTHOR_FIXTURE_1.getName() + ".*"));
		window.button(JButtonMatcher.withName("addBookButton")).click();

		await().atMost(TIMEOUT_SECONDS, TimeUnit.SECONDS).untilAsserted(
				() -> assertThat(window.label("bookErrorLabel").text()).contains(BOOK_FIXTURE_1.getId()));
	}
	...
	public void testDeleteAuthorAndAssociatedBooks() {
		...
		await().atMost(TIMEOUT_SECONDS, TimeUnit.SECONDS)
				.untilAsserted(() -> assertThat(window.table("booksTable").contents()).isEmpty());
	}

	private MongoCollection<Author> getAuthorCollection() {
		return client.getDatabase(DB_NAME).getCollection(DB_AUTHOR_COLLECTION, Author.class)
				.withCodecRegistry(pojoCodecRegistry);
	}

	private MongoCollection<Book> getBookCollection() {
		return client.getDatabase(DB_NAME).getCollection(DB_BOOK_COLLECTION, Book.class)
				.withCodecRegistry(pojoCodecRegistry);
	}
