...
public class BookManagerSwingViewIT extends AssertJSwingJUnitTestCase {
	...
	public void testShowAllAuthors() {
		...
		await().atMost(TIMEOUT_SECONDS, TimeUnit.SECONDS).untilAsserted(() -> {
			assertThat(window.list("authorsList").contents()).containsExactly(danBrown.getName(),
	...
	public void testDeleteAuthorSuccess() {
		GuiActionRunner.execute(() -> controller.addAuthor(new Author("1", "George Orwell")));
	...
	public void testDeleteBookSuccess() {
		Author georgeOrwell = new Author("1", "George Orwell");
		GuiActionRunner.execute(() -> view.getAuthorListModel().addElement(georgeOrwell));
		GuiActionRunner.execute(() -> controller.addBook(new Book("1", "Animal Farm", 93, georgeOrwell.getId())));
	...
	public void testDeleteBookError() {
		...
		GuiActionRunner.execute(() -> view.getBookTableModel().addElement(animalFarm, georgeOrwell));
